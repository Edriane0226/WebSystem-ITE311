<title><?= esc($course['courseTitle'] ?? 'Course Assignments') ?></title>
	...
	<div class="row g-4">
			...
			<?php if (!empty($assignments)): ?>
				<?php foreach ($assignments as $assignment): ?>
					<?php
						...
						$publishDate = !empty($assignment['publishDate']) ? date('M d, Y', strtotime($assignment['publishDate'])) : 'Not published yet';
						$dueDate = !empty($assignment['dueDate']) ? date('M d, Y', strtotime($assignment['dueDate'])) : 'No due date';
						$isOverdue = !empty($assignment['dueDate']) && strtotime($assignment['dueDate']) < time();
							...
							<div class="d-flex flex-wrap gap-2 mt-3">
								...
								<?php if ($role === 'student'): ?>
									...
									<?php if ($allowedAttempts && $studentAttemptsCount >= $allowedAttempts): ?>
										<span class="badge bg-warning text-dark align-self-center">Max attempts used</span>
									<?php endif; ?>
								<?php elseif ($submissionCount > 0): ?>
									<span class="badge bg-success align-self-center"><?= $submissionCount ?> submission(s) received</span>
								<?php endif; ?>
							</div>
		...
		<div class="col-lg-4">
			<div class="card border-0 shadow-sm mb-4">
				<div class="card-body">
					<h6 class="fw-semibold mb-3">Course At A Glance</h6>
			...
			<?php else: ?>
				<div class="card border-0 shadow-sm">
					<div class="card-body">
						...
						<p class="small text-muted mb-0">Reach out to your instructor for clarifications on submission requirements.</p>
					</div>
				</div>
			<?php endif; ?>
		</div>
